...
class Bayes(object):
    ...
    def train(self, X, Y):
		...
		#print "k2",  self.condProbs[self.classes[Y[i]]][j], self.posPerFeature[j][feature]
    ...
    def getModifiedPredictions(self, X, Y):
        ...
        for p in predictions:
            argMaxpredictions.append(p)
        Y_ = [ self.classes[y] for y in Y]
        return (np.array(argMaxpredictions), Y_)

    def getConfusionMatrix(self, X, Y):
        (argMaxpredictions, Y_) = self.getModifiedPredictions(X, Y)
        argMaxpredictions = np.argmax(argMaxpredictions, axis=1)
